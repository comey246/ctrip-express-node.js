...
class orderModel {
    ...
    static getMap = (id)=>{
        try{
            if(mapDatabase.ordersMap[id])
                console.log(mapDatabase.ordersMap[id])
            return  mapDatabase.ordersMap[id]
        }catch (err){
            console.error('order get orders Fail',err)
            return null
        }
    }
    static getAllMap = (count,total)=>{
        try{
            const map = Object.keys(orderDatabase.orders)
            const all = count  + total ;
            let arr
            if (all < map.length){
                 arr = map.slice(total,count + total)
            }else {
                 arr = map.slice(total,map.length - total)
            }
            return arr.reverse()
        }catch (err){
            console.error('order get all orders Fail',err)
            return null
        }
    ...
    static deletMap = (id,order_id)=>{
        try{
            const OrderId = mapDatabase.ordersMap[id]
            const index = OrderId.indexOf(order_id);
            OrderId.splice(index, 1)
            mapDatabase.ordersMap[id] = OrderId
            this.saveDatabase(mapDatabase,mapPath)
            return true
        }catch (err){
            console.error('order get orders Fail',err)
